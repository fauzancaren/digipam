...
class Model_app extends CI_Model
{
	...
	*/
	...
	function get_next_id_asset($code)
	{
		$query = $this->db->query("SELECT CAST((ifnull(MAX(SUBSTRING(AssetListCode, -4, 4)),0) + 1) AS SIGNED) as max FROM TblAssetListing where AssetTypeRef='" . $code['AssetId'] . "'")->result();
		foreach ($query as $rows) {
			$id = $rows->max;
			switch (strlen($id)) {
				case 1:
					$nextid = $code['codeAsset'] . "000" . $id;
					return $nextid;
				case 2:
					$nextid = $code['codeAsset'] . "00" . $id;
					return $nextid;
				case 3:
					$nextid = $code['codeAsset'] . "0" . $id;
					return $nextid;
				case 4:
					$nextid = $code['codeAsset'] . "" . $id;
					return $nextid;
				default:
					$nextid = $code['codeAsset'] . "0000";
					return $nextid;
			}
		}
	}

	function get_next_id_assetDiv($code)
	{
		$query = $this->db->query("SELECT CAST((ifnull(MAX(SUBSTRING(AssetListDivisi, -4, 4)),0) + 1) AS SIGNED) as max FROM TblAssetListing where assetDivisiIdRef='" . $code['assetDivisiId'] . "'")->result();
		foreach ($query as $rows) {
			$id = $rows->max;
			switch (strlen($id)) {
				case 1:
					$nextid = $code['assetDivisiCode'] . "/000" . $id;
					return $nextid;
				case 2:
					$nextid = $code['assetDivisiCode'] . "/00" . $id;
					return $nextid;
				case 3:
					$nextid = $code['assetDivisiCode'] . "/0" . $id;
					return $nextid;
				case 4:
					$nextid = $code['assetDivisiCode'] . "/" . $id;
					return $nextid;
				default:
					$nextid = $code['assetDivisiCode'] . "/0000";
					return $nextid;
			}
		}
	}
	...
	function get_base_64_by_id($id)
	{
		...
		try {
			if (file_exists($configfile . '/employee/' . $id . '.png')) {
				...
				$path = $configfile . '/obi-erp/user.png';
				$type = pathinfo($path, PATHINFO_EXTENSION);
				$data = file_get_contents($path);
				$base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
				return $base64;
