...
<head>
	...
	<link href="<?= base_url("asset/mainmenu.css?version=1.7") ?>" rel="stylesheet" type="text/css">
    ...
    <link href="<?= base_url("asset/jquery/jquery-ui-1.13.2/jquery-ui.min.css")?>" rel="stylesheet"  type="text/css">  
	...
	<script src="<?= base_url("asset/jquery/jquery-ui-1.13.2/jquery-ui.min.js") ?>" type="text/javascript"></script> 
...
<body class="loaded">
	...
	<nav class="navbar navbar-expand-md navbar-light ">
		<div class="container-fluid">
			<a class="navbar-brand fw-bold">
				<img src="<?= base_url("asset/image/obi-erp/logo.png") ?>" width="30" height="30" class="d-inline-block align-top" alt="">
			...
			<div class="d-flex">
				...
				<div class="btn-group  hover-gray">
					...
					<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink" data-bs-popper="none">
						...
						<li><a href="<?= base_url("login/logout") ?>" class="dropdown-item">Keluar</a></li>
					</ul>
				</div>
			...
			<form class="modal-content" name="reset-password">
				...
				<div class="modal-body">
					<div class="row justify-content-center">
						<div class="col-11">
							<div class="row mb-1">
								<label for="txt-new-password" class="col-sm-3 col-form-label">Password Baru</label>
								<div class="col-sm-9">
	...
	<script>
		$(document).ready(function() {
			...
			$(function() {
				$("form[name='reset-password']").validate({
					...
					submitHandler: function(form) {
						...
						$.ajax({
							method: "POST",
							url: "<?= base_url("function/client_data_master/reset_password") ?>",
							data: {
			...
			$.ajax({
				url: "<?= base_url("client/get_side_menu") ?>",
			...
			function set_page_by_menu(id,menu, elm) {
				...
				$.ajax({
					url: "<?= base_url("client/get_content/") ?>" + menu + "/" + elm+ "/" + id,
					success: function(result) {
						remove_fixed_header();
	...
	<script>  
		...
		// 			$.ajax({
		// 				method: "POST",
		// 				url: "<?= base_url("client/update_notification/") ?>" + $(this).data("id"),
		// 				success: function() {
		...
		// 				"NotifId": last_id
		// 			},
		// 			url: "<?= base_url("client/get_notification") ?>",
		// 			success: function(data) {
		...
		// 		}
		// 	});
		// 	const socket = io.connect('https://wa.obi-system.com:2000');
		// 	dataUser = <?= JSON_ENCODE($_users) ?>;
		// 	var countOnline = 0;
		// 	var countOffline = 0;
		// 	socket.on('connect', function () {   
		// 		socket.emit('login', {
		// 			"code":"<?= $_sessionuser["MsEmpCode"]?>",
		// 			"name":"<?= $_sessionuser["MsEmpName"]?>",
		// 			"uuid":"<?= $_sessionuser["login_uuid"]?>"
		// 		});
		// 		// PERTAMA DIJALANKAN DAN AKAN MEMBACA SEMUA USER YANG SEDANG AKTIF
		// 		socket.emit("get_all_socket");
		// 		socket.on('all_socket', function(data) {  
		// 			for(var i=0; i < data.length;i++){ 
		// 				changelist(data[i].code,true);
		// 			}
        //     	}); 

		// 		// MENGUPDATE DATA USER YANG BARU AKTIF
		// 		socket.on("new_login", async (data) => {
		// 			console.log("New Online : " + data.code);
		// 			if("<?= $_sessionuser["MsEmpCode"] ?>" != data.code) changelist(data.code,true); 
		// 		});

		// 		// MENGUPDATE DATA USER YANG BARU LOGOUT
		// 		socket.on("logout", async (data) => {
		// 			console.log("Logout : " + data.code);
		// 			if("<?= $_sessionuser["MsEmpCode"] ?>" != data.code) changelist(data.code,false); 
		// 		});
		...
		// 					body: data.message ,
		// 					icon: '<?= base_url("asset/image/obi-erp/logo.png")?>',
		...
		// 				Toast.fire({ 
		// 					html: '<div class="d-flex"><div><img src="<?= base_url()?>/asset/image/obi-erp/pesan1.webp" height="50" width="60" class="me-2"></div><div><b>Ada pesan baru !!!</b><br><span class="text-muted" style="font-size:0.75rem">' + data.name + " : <br>" + data.message + "</span></div></div>", 
		// 					showConfirmButton: true,
